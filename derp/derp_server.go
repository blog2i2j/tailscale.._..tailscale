...
var verboseDropKeys = map[unique.Handle[key.NodePublic]]bool{}
...
type Server struct {
	...
	clients  map[unique.Handle[key.NodePublic]]*clientSet
	...
	// local).
	clientsMesh map[unique.Handle[key.NodePublic]]PacketForwarder
	...
	peerGoneWatchers map[unique.Handle[key.NodePublic]]map[unique.Handle[key.NodePublic]]func(unique.Handle[key.NodePublic])

	// maps from netip.AddrPort to a client's public key
	keyOfAddr map[netip.AddrPort]unique.Handle[key.NodePublic]
...
type PacketForwarder interface {
	ForwardPacket(src, dst unique.Handle[key.NodePublic], payload []byte) error
	String() string
}
...
//
// s.mu must be held.
func (s *Server) broadcastPeerStateChangeLocked(peer unique.Handle[key.NodePublic], ipPort netip.AddrPort, flags PeerPresentFlags, present bool) {
...
func (s *Server) addPeerGoneFromRegionWatcher(peerA, peerB unique.Handle[key.NodePublic], f func(unique.Handle[key.NodePublic])) {
	s.mu.Lock()
	defer s.mu.Unlock()
	m, ok := s.peerGoneWatchers[peerA]
	if !ok {
		m = map[unique.Handle[key.NodePublic]]func(unique.Handle[key.NodePublic]){}
...
func (s *Server) removePeerGoneFromRegionWatcher(peerA, peerB unique.Handle[key.NodePublic]) {
	s.mu.Lock()
	defer s.mu.Unlock()
...
func (s *Server) notePeerGoneFromRegionLocked(handle unique.Handle[key.NodePublic]) {
...
// sent one recently.
func (c *sclient) requestPeerGoneWriteLimited(peer unique.Handle[key.NodePublic], contents []byte, reason PeerGoneReasonType) {
...
func (s *Server) recordDrop(packetBytes []byte, srcHandle, dstHandle unique.Handle[key.NodePublic], reason dropReason) {
...
func (c *sclient) onPeerGoneFromRegion(peer unique.Handle[key.NodePublic]) {
...
func (c *sclient) requestPeerGoneWrite(peer unique.Handle[key.NodePublic], reason PeerGoneReasonType) {
	select {
...
func (s *Server) recvPacket(br *bufio.Reader, frameLen uint32) (dstHandle unique.Handle[key.NodePublic], contents []byte, err error) {
	if frameLen < keyLen {
...
var zpubHandle unique.Handle[key.NodePublic] = zpub.Handle()

func (s *Server) recvForwardPacket(br *bufio.Reader, frameLen uint32) (srcHandle, dstHandle unique.Handle[key.NodePublic], contents []byte, err error) {
...
type sclient struct {
	...
	nc             Conn
	key            key.NodePublic
	handle         unique.Handle[key.NodePublic] // handle is a cached handle for key
	info           clientInfo
	logf           logger.Logf
	...
	sawSrc set.Set[unique.Handle[key.NodePublic]]
	bw     *lazyBufioWriter
...
type pkt struct {
	...
	src unique.Handle[key.NodePublic]
}
...
func (c *sclient) sendPacket(srcHandle unique.Handle[key.NodePublic], contents []byte) (err error) {
	defer func() {
		// Stats update.
		if err != nil {
...
//
// It must only be called from the sendLoop goroutine.
func (c *sclient) noteSendFromSrc(src unique.Handle[key.NodePublic]) {
...
func (s *Server) AddPacketForwarder(dst unique.Handle[key.NodePublic], fwd PacketForwarder) {
	s.mu.Lock()
	defer s.mu.Unlock()
...
// fwd must be comparable.
func (s *Server) RemovePacketForwarder(dst unique.Handle[key.NodePublic], fwd PacketForwarder) {
	s.mu.Lock()
	defer s.mu.Unlock()
...
func (f *multiForwarder) ForwardPacket(src, dst unique.Handle[key.NodePublic], payload []byte) error {
